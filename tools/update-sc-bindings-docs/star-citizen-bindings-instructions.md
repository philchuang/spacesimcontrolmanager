...
2. Read `C:\Program Files\Roberts Space Industries\StarCitizen\{STAR_CITIZEN_ENVIRONMENT}\data\Localization\english\global.ini`
   1. `STAR_CITIZEN_ENVIRONMENT` defaults to `LIVE` in `sc_bindings_core.py`
   2. This is a keyed list of strings, with one key-value pair per line, delimited by the first `=` character
   3. Iterate over the actions list from the previous step
      1. For each `label` and `categoryLabel`, look up the key in the global.ini, and if found replace it with the value
3. Read `C:\Program Files\Roberts Space Industries\StarCitizen\{STAR_CITIZEN_ENVIRONMENT}\USER\Client\0\Profiles\default\actionmaps.xml`
   1. Iterate on `<actionMap>` elements
